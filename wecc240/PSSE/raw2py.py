...
BUSID = True # keep original bus ID
REFRESH = True # regenerate all files
...
import pypower.idx_cost as pp_gencost
import pypower.idx_dcline as pp_dcline
import pypower.idx_cost as pp_dclinecost
...
class PSSEraw:
    ...
    def to_pypower(self,file=None):
        ...
        bus_map = {}
        bus_name = {}
        bus_kv = [x[psse.bus.BASEKV] for x in self.section["BUS DATA"]]
        ...
        for n,m in enumerate(self.section["BRANCH DATA"]):
            ...
            RATE_A,RATE_B,RATE_C = m[psse.branch.RATE1:psse.branch.RATE3+1]
            # note that STE rating (RATE_C) can be less than LTE rating (RATE_B)
            if RATE_A == 0: # all zero line ratings not supported by pypower so none are allowed
                RATE_A = min(RATE_B,RATE_C)
            TAP = 0
            SHIFT = 0.0
            ...
            # include branch shunt in from and to busses when branch is active
            if BR_STATUS == 1: 
                ...
                if tbus not in Gs:
                    Gs[tbus] = 0.0
                    Bs[tbus] = 0.0
                zpufrom = bus_kv[fbus]**2 / self.mvabase 
                zputo = bus_kv[fbus]**2 / self.mvabase
                Gs[fbus] += gi / zpufrom
                Bs[fbus] += bi / zpufrom
                Gs[tbus] += gj / zputo
                Bs[tbus] += bj / zputo
        ...
        for n,m in enumerate(self.section["TRANSFORMER DATA"]):
            ...
            BR_R = round(m[psse.transformer.R12],9)
            BR_X = round(m[psse.transformer.X12],9)
            BR_B = round(m[psse.transformer.MAG2],9)
        ...
        # save case for pypower
        self.case = deepcopy(case)
        for tag,data in self.case.items():
            if isinstance(data,list):
                self.case[tag] = np.array(data)


        # rename busses
        if BUSID:
            # print(case["bus"])
            bus_id = {x[pp_bus.BUS_I]:bus_index[n] for n,x in enumerate(case["bus"])}
            def remap(data,column):
                x = np.array(data)
                x[:,column] = [bus_id[y] for y in x[:,column]]
                return x.tolist()
            case["bus"] = remap(case["bus"],pp_bus.BUS_I)
            case["gen"] = remap(case["gen"],pp_gen.GEN_BUS)
            case["branch"] = remap(case["branch"],pp_branch.F_BUS)
            case["branch"] = remap(case["branch"],pp_branch.T_BUS)
        ...
        # write cost file
        if not os.path.exists(self.name+"_cost.csv") or os.path.getmtime(self.name+"_gentypes.csv") > os.path.getmtime(self.name+"_cost.csv") or REFRESH:

...
def _(mo):
    mo.md(r"""
    This notebook reviews the `county_total.csv` data imported from the `loads` package `Total` module.
    """)
    return


@app.cell
def _(mo, read_county_total):
    data = read_county_total().set_index("timestamp")
...
@app.cell
def _(help_md, mo, read_county_total):
    mo.accordion(help_md(read_county_total))
    return
...
def _():
    ...
    from wecc240_data import read_county_total, help_md

    return help_md, mo, plt, read_county_total

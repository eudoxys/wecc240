...
def _(mo):
    mo.md(r"""
    This notebook reviews the `node_net.csv` data.
    """)
    return


@app.cell
def _(read_bus_gis, read_node_net):
    data = read_node_net().set_index("timestamp")
...
@app.cell
def _(help_md, mo, read_node_net):
    mo.accordion(help_md(read_node_net))
    return
...
def _():
    ...
    from wecc240_data import read_node_net, read_bus_gis, help_md

    return help_md, mo, plt, read_bus_gis, read_node_net
